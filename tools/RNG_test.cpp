...
class DummyRNG : public PractRand::RNGs::vRNG64 {
public:
    //declare state
    uint8_t key[16];
    uint64_t counter;
    ...
    uint64_t raw64() {
        uint64_t out;
        siphash((void*)&counter, sizeof counter, (void*)key, (uint8_t*)(void*)&out, sizeof out);
        counter += 1;
        return out;
    }
    ...
    std::string get_name() const {return "SipHash-2-4";}
};
/*
